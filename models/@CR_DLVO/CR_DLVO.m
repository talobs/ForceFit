classdef CR_DLVO < model
    ...
    methods
        function obj = CR_DLVO(handles, group_idx)
            obj.name = 'CR DLVO' ;
            obj.group_idx = group_idx ;
            obj.cal_fun = @CR_DLVO_fun ;
            obj.fit_fun = @CR_DLVO_fit ;
            obj.cal_p_fun = @CR_DLVO_cal_p ;
            ...
            obj.fp.C.mval = .72 ;
            obj.fp.C.aval_guess = .9 ;
            obj.fp.C.aval_min = .833 ;
            obj.fp.C.aval_max = 1 ;
            ...
            if ~isempty(obj.pr.n.val) && ~isempty(obj.pr.T_C.val)
                ...
                obj.fp.ld.aval_min = cal_ld(obj.pr.n.val*1.1, obj.pr.T_C.val) ;
                obj.fp.ld.aval_max = cal_ld(obj.pr.n.val*0.9, obj.pr.T_C.val) ;
            end
            
            obj.fbnd = struct('min', fitbnd('min', 2), 'max', fitbnd('max', 20)) ;
            
            if obj.fp.ld.aval_guess > 2
                obj.fbnd.min.val = 1.5*obj.fp.ld.aval_guess ;
            else
                obj.fbnd.min.val = 3 ;
            end
