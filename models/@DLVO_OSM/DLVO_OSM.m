classdef DLVO_OSM < model
    ...
    methods
        function obj = DLVO_OSM(handles, group_idx)
            obj.name = 'DLVO_OSM' ;
            obj.group_idx = group_idx ;
            obj.cal_fun = @DLVO_fun ;
            obj.fit_fun = @DLVO_fit ;
            obj.cal_p_fun = @DLVO_OSM_cal_p ;
            ...
            if isfield(handles.groups(group_idx), 'osmolyte')
                obj.pr.osm.val = handles.groups(group_idx).osmolyte ;
            end
            
            if isfield(handles.groups(group_idx), 'n_o')
                obj.pr.osm_n.val = handles.groups(group_idx).n_o ;
            end
